...
export function detectUserCountry(): Country {
  try {
    ...
    if (tz) {
      ...
      if (tz.startsWith('Africa/')) {
        ...
        if (city === 'Harare') return COUNTRIES.find((c) => c.code === 'ZW') || COUNTRIES[COUNTRIES.length - 1];
        if (city === 'Johannesburg') return COUNTRIES.find((c) => c.code === 'ZA') || COUNTRIES[COUNTRIES.length - 1];
      }
    }
  } catch {
    // ignore
  ...
  const defaultZim = COUNTRIES.find((c) => c.code === 'ZW');
  return defaultZim || COUNTRIES[0];
}
