...
- **Complete Elimination of Dummy Data & Sample Leads (`index.tsx`, `useLeadStore.ts`):** Removed all hardcoded sample leads (`INITIAL_SAMPLE_LEADS` featuring Dr. T. Sithole, Alex Rivera, Clara Ndlovu, Marcus Chen) and simulated lead pools from `useLeadStore`. Added automatic storage migration and rehydration filters (`onRehydrateStorage` + `useEffect` cleanup) that instantly purge legacy sample leads from existing device storage upon mount. Completely removed the "Daily Pulse" footer overview and mock metric widgets from the Home screen, leaving a clean, high-craft, 100% authentic live Radar feed with zero dummy artifacts.
...
- **Live Stats Ribbon & Glassmorphism Blur Navigation (`index.tsx`, `_layout.tsx`, `LeadRow.tsx`, `DealCard.tsx`):** Added a top Live Stats Ribbon on Home computing real pipeline value, monitored channel status, and speed metrics. Upgraded the floating bottom navigation bar with hardware-accelerated `expo-blur` (`BlurView`) over translucent midnight navy with crisp glass borders. Fixed lead row rendering where WhatsApp channel names were previously truncated with `.split(' ')[0]` and concatenated directly after sender names (appearing as "Mikana"), separated channel attribution into a dedicated badge with group icons, and eliminated ambiguous generic "Regional / On-Site" location defaults.

## [1.6.0] - 2026-09-04

### Onboarding Expansion: Notifications, Paywall & 6-Stage Stepper
- **WhatsApp Group Detection Resilience & Relay Session Recovery (`groups.tsx`, `server/index.js`, `whatsappRelay.ts`):** Resolved failure where detected groups stopped loading due to Render relay cold-starts and in-memory session disconnects. Added client-side session auto-awakening (`ensureSessionReady`), wrapped server Baileys socket queries in 6-second `Promise.race` timeouts with background pre-caching upon connection handshake, added an actionable "WhatsApp Session Offline" card with 1-tap re-linking back to `pair.tsx`, guaranteed preservation of existing saved `radarChannels` as fallback cards so user progress is never wiped, and provided an inline manual group entry input (`[ + Add Group Name ]`) to allow instant channel monitoring without blocking onboarding progress.
- **Checked Groups & Settings Reload Persistence (`groups.tsx`, `discover.tsx`, `useSettingsStore.ts`, `useCatalogStore.ts`):** Resolved bug where checked trade groups and setup inputs were wiped out upon app reload. Configured real-time persistence on checkbox toggle, bi-directional auto-matching against `radarChannels` when group rosters stream from the relay, reactive `useEffect` synchronization when `AsyncStorage` completes hydration, and automatic synchronization of business name into `useCatalogStore` so the Business Hub instantly reflects the user's business profile.
...
- **Notification Permission Re-Prompt Elimination (`notifications.tsx`):** Added mount-level system permission check (`Notifications.getPermissionsAsync()`) and `enablePushNotifications` store detection. If permissions are already granted or activated, the screen immediately renders a confirmed active state with a direct `"Continue to Membership Plan"` action, completely eliminating redundant OS permission dialogs and re-accept prompts when navigating or reloading.
- **Membership Tier & Paywall Redesign (`onboarding/paywall.tsx` & `modal/paywall.tsx`):** Overhauled Step 6 onboarding paywall and in-app paywall modal with 100% NativeWind v4 styling. Replaced ambiguous card selection with direct, actionable buttons right on each tier card (`[ Activate Pro — $9.99 / month → ]` and `[ Continue with Free Tier ($0) → ]`). Added a clean billing cycle toggle switching between Monthly ($9.99/mo) and Annual ($79.99/yr · Save 35%), eliminating all conflicting trial text and unhandled selection dead-ends.
- **RevenueCat Sandbox Simulation Mode Guarantee:** Ensured development and hackathon review builds without active Google Play billing gracefully activate Pro entitlements in Sandbox Mode with 1 tap, eliminating "dummy" paywall failures and transaction hangs.
